...
import { getCurrentCourseProgressAPI, markLectureAsViewedAPI, resetCourseProgressAPI } from '@/services/allAPI';
...
const CourseProgress=()=> {
  ...
  const  fetchCurrentCourseProgress=async()=> {
    console.log("User ID:", auth?.user?.id, "Course ID:", id);
    const response = await getCurrentCourseProgressAPI(auth?.user?.id, id);
    if (response?.success) {
  ...
  const  updateCourseProgress=async()=> {
    if (currentLecture) {
      const response = await markLectureAsViewedAPI(
        auth?.user?.id,
  ...
  const  handleRewatchCourse=async()=> {
    const response = await resetCourseProgressAPI(
      auth?.user?.id,
      studentCurrentCourseProgress?.courseDetails?._id
    );
  ...
  return (
    <div className="flex flex-col bg-[#1c1d1f] text-white pt-16">
